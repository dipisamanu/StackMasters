...
<head>
    ...
    <title>Restituzione Libー - StackMasters</title>
...
<script>
    ...
    // Oggetto temporaneo per salvare i dati del libro correntemente scansionato
    ...
    async function fetchBookInfo(code) {
        try {
            ...
            if(data.success) {
                // Salviamo i dati per usarli nell'inserimento lista
                currentBookData = data;
                openModal();
            } else {
                showStatus(data.error || "Volume non identificato", "error");
                returnInp.value = "";
                returnInp.focus();
            }
        } catch(e) {
    ...
    function closeModal() {
        ...
        returnInp.value = "";
        returnInp.focus();
        currentBookData = null; // Reset dati
    }

    function confirmCondition(cond) {
        const comment = document.getElementById('modal-comment').value;
        // Passiamo solo ID e Cover alla funzione di aggiunta lista (titolo rimosso)
