...
// 1. Inclusione configurazione sessione standard del progetto
...
<head>
    ...
    <style>
        ...
        /* Stile per bottoni disabilitati nella modale */
        button:disabled { opacity: 0.4; cursor: not-allowed; background-color: #f8fafc !important; border-color: #e2e8f0 !important; }
        button:disabled .group-hover\:text-green-500 { color: #a3a3a3 !important; }
    </style>
</head>
<!-- CORREZIONE: Aggiunte classi flex per gestire il layout e spingere il footer in basso -->
<body class="flex flex-col h-full">

<!-- CORREZIONE: Aggiunta classe flex-grow per far occupare al main tutto lo spazio disponibile -->
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-center">Identifica Copia Fisica</label>
                    ...
                    <div class="bg-emerald-600 p-6 rounded-2xl text-white shadow-xl">
                        <h4 class="text-[10px] font-black uppercase tracking-widest text-emerald-100 mb-4 opacity-80">Protocollo Rientro</h4>
                        <ul class="text-xs space-y-3 font-medium">
                            <li class="flex items-start gap-3"><i class="fas fa-clock mt-0.5"></i> Tolleranza: 3 giorni solari</li>
                            <li class="flex items-start gap-3"><i class="fas fa-coins mt-0.5"></i> Penale Ritardo: 0.50€ / giorno</li>
                            <li class="flex items-start gap-3"><i class="fas fa-tools mt-0.5"></i> Valutazione danni obbligatoria</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full overflow-hidden min-h-[500px]">
                        <div class="p-5 border-b bg-slate-50/50 flex justify-between items-center">
                            <h3 class="font-black text-slate-700 uppercase text-xs tracking-widest">Lista Volumi Rientrati</h3>
                            <span id="returns-count" class="bg-emerald-500 text-white text-[10px] px-2.5 py-1 rounded-full font-black">0</span>
                        </div>
                        ...
                        <div id="scanned-returns-list" class="flex-1 overflow-y-auto p-4 space-y-3">
                            <div id="empty-return-msg" class="h-full flex flex-col items-center justify-center text-slate-300 py-20">
                                ...
                                <p class="font-black uppercase text-[10px] tracking-widest text-slate-400">Scansiona o digita i volumi</p>
                            </div>
                        </div>
...
<div id="evaluation-modal" class="modal fixed inset-0 bg-slate-900/80 backdrop-blur-sm hidden items-center justify-center p-4 z-50">
    <div class="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md border-t-8 border-emerald-500">
        ...
        <div class="space-y-3 mb-6">
            <button type="button" data-condition="BUONO" onclick="confirmCondition('BUONO')" class="w-full flex items-center justify-between p-4 bg-green-50 border-2 border-green-200 rounded-2xl hover:bg-green-100 transition-all group focus:ring-4 focus:ring-green-200 outline-none">
                <div class="text-left">
            ...
            <button type="button" data-condition="USURATO" onclick="confirmCondition('USURATO')" class="w-full flex items-center justify-between p-4 bg-blue-50 border-2 border-blue-200 rounded-2xl hover:bg-blue-100 transition-all group focus:ring-4 focus:ring-blue-200 outline-none">
                <div class="text-left">
            ...
            <button type="button" data-condition="DANNEGGIATO" onclick="confirmCondition('DANNEGGIATO')" class="w-full flex items-center justify-between p-4 bg-yellow-50 border-2 border-yellow-200 rounded-2xl hover:bg-yellow-100 transition-all group focus:ring-4 focus:ring-yellow-200 outline-none">
                <div class="text-left">
            ...
            <button type="button" data-condition="SMARRITO" onclick="confirmCondition('SMARRITO')" class="w-full flex items-center justify-between p-4 bg-red-50 border-2 border-red-200 rounded-2xl hover:bg-red-100 transition-all group focus:ring-4 focus:ring-red-200 outline-none">
                <div class="text-left">
        ...
        <textarea id="modal-comment" class="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm focus:border-emerald-500 outline-none transition-all" rows="2" placeholder="Note per l'utente (opzionale)..."></textarea>

        <button type="button" onclick="closeModal()" class="w-full mt-4 text-slate-400 font-bold text-[10px] uppercase hover:text-slate-600 transition-colors">Annulla operazione</button>
    </div>
</div>
...
<script>
    ...
    async function fetchBookInfo(code) {
        try {
            ...
            if(responseData.success) {
                ...
                openModal(currentBookData.condizione); // Passa la condizione di partenza
            } else {
    ...
    function openModal(startCondition = 'BUONO') {
        ...
        modal.querySelectorAll('.space-y-3 button').forEach(button => {
            ...
            button.disabled = false; // Abilita di default
            if (buttonLevel < startLevel) {
                button.disabled = true; // Disabilita se la condizione è migliore di quella di partenza
            }
        });
