...
-- 2. TABELLE PRINCIPALI (LIBRO E UTENTE)
...
(
    ...
    FOREIGN KEY (lingua_originale_id) REFERENCES Lingua (id)
);
...
(
    ...
    scadenza_verifica       TIMESTAMP          NULL, -- Gestito da logica app (NOW() + 1h)
...
(
    ...
    FOREIGN KEY (id_genere) REFERENCES Genere (id) ON DELETE CASCADE,
    FOREIGN KEY (id_libro) REFERENCES Libro (id_libro) ON DELETE CASCADE
);
...
(
    ...
    copia_libro          INT       NULL,     -- FK Inventario: popolato solo quando la copia viene assegnata
    ...
    FOREIGN KEY (id_utente) REFERENCES Utente (id_utente),
    FOREIGN KEY (id_libro) REFERENCES Libro (id_libro),
    FOREIGN KEY (copia_libro) REFERENCES Inventario (id_inventario)
);
