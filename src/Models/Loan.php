...
class Loan
{
    ...
    public function registraRestituzione(int $inventarioId, string $condizioneRientro, ?string $dannoCommento = null): array
    {
        ...
        try {
            ...
            $livelloPartenza = $condizioniMap[$prestito['condizione_partenza']] ?? 0;
            ...
            return ['status' => 'success', 'multa_generata' => (float)$multaTotale, 'messaggi' => $messaggi, 'assegnato_a_prenotazione' => $successore !== null];
        } catch (Exception $e) {
            $this->db->rollBack();
            throw $e;
        }
    ...
    private function assegnaProssimaPrenotazione(int $libroId, int $inventarioId): ?array {
        ...
        $this->db->prepare("UPDATE prenotazioni SET copia_libro = ?, data_disponibilita = NOW(), scadenza_ritiro = ? WHERE id_prenotazione = ?")->execute([$inventarioId, $scad, $pren['id_prenotazione']]);
    ...
    private function gestisciPrenotazioniPrimaDelPrestito(int $libroId, int $inventarioId, int $utenteId): void {
        $this->db->prepare("DELETE FROM prenotazioni WHERE id_libro = ? AND id_utente = ? AND copia_libro IS NULL")->execute([$libroId, $utenteId]);
    ...
    private function aggiornaStatisticheRestituzione(int $utenteId, int $ritardo): void {
        $this->db->prepare("UPDATE utenti_ruoli SET streak_restituzioni = CASE WHEN :r = 0 THEN streak_restituzioni + 1 ELSE 0 END WHERE id_utente = :u")->execute(['r' => $ritardo, 'u' => $utenteId]);
    }
}